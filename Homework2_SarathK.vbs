Sub Button2_Click()
    ...
    For Each ws In Worksheets
        ...
        ws.Cells(1, 10).Value = "Yearly Change in Stock price"
        ws.Cells(1, 11).Value = "Percent Change"
        ws.Cells(1, 12).Value = "Total Stock Volume"

        'Variables
        Dim ticker_symbol As String ' for stock ticker
        Dim total_vol As Double ' for total volume of stocks transacted
        total_vol = 0 ' initializing total stocks variable
        Dim rowcount As Long ' track each stock symbol
        rowcount = 2
        Dim year_open As Double ' variable to hold year open price
        year_open = 0
        Dim year_close As Double 'Variable for year end close
        year_close = 0
        Dim year_change As Double ' Variable for price change
        year_change = 0
        Dim percent_change As Double 'Variable for percent change
        percent_change = 0
        Dim lastrow As Long ' Variable for total rows to loop through
        
        lastrow = ws.Cells(Rows.Count, 1).End(xlUp).Row ' returns total rows
        ...
        For i = 2 To lastrow
            ...
            If ws.Cells(i, 1).Value <> ws.Cells(i + 1, 1).Value Then
                ...
                'Calculate the price change for the year and copy to summary
